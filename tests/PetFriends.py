...
def test_add_pets_with_valid_data(name='Galka', animal_type='bird', age='1', pet_photo='galka.png'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pets(api_key, 'bob', 'jiraffe', '5', 'bob.jpg')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_add_pets_with_valid_data_without_photo(name='<Барсик>БезФото', animal_type='кот', age='6'):
...
def test_add_photo_to_pet_with_valid_data(pet_photo='jojo.jpeg'):
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet_simple(auth_key, "без фото", "кот", "11")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_add_pet_negative_age_number(name='galka', animal_type='bird', age='-3', pet_photo='galka.png'):
...
def test_add_pet_with_four_digit_age_number(name='galka', animal_type='bird', age='9000', pet_photo='glaka.png'):
...
def test_add_pet_with_empty_value_in_variable_name(name='', animal_type='bird', age='1', pet_photo='galka.png'):
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='bird', age='1', pet_photo='galka.png'):
    ...
    name = 'Напу Амо Хала Она Она Анека Вехи Вехи Она Хивеа Нена Вава Кехо'
...
def test_add_pet_with_special_characters_in_variable_animal_type(name='galka', age='1', pet_photo='galka.png'):
...
def test_add_pet_with_numbers_in_variable_animal_type(name='galka', animal_type='34562', age='9',
...
def test_add_pet_with_a_lot_of_words_in_variable_animal_type(name='galka', age='2', pet_photo='galka.png'):
...
def test_get_api_key_with_wrong_password_and_correct_mail(email=valid_email, password=valid_password + 'qwerty'):
...
def test_get_api_key_with_wrong_email_and_correct_password(email=valid_email + 'qwerty', password=valid_password):
    '''Проверяем запрос с невалидным паролем и с валидным емейлом.
    Проверяем нет ли ключа в ответе'''
    status, result = pf.get_api_key(email, password)
    assert status == 403
